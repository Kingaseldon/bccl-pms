                    ...
                    <div class="col-sm-9 card dashboard-product">
                                ...
                                <table style="width:100%;">
                                    @foreach ($details as $userDetail)
                                        ...
                                        @if ((bool) $userDetail->Qualification1 || (bool) $userDetail->Qualification2)
                                            <tr>
                                                <th style="width:35%;">Qualification<span
                                                        class="pull-right d-none d-sm-block">:</span> &nbsp;&nbsp;</th>
                                                <td style="padding-left:25px;">
                                                    @if ((bool) $userDetail->Qualification1)
                                                        {{ $userDetail->Qualification1 }}
                                                        @endif @if ((bool) $userDetail->Qualification2)
                                                            @if ((bool) $userDetail->Qualification1)
                                                                <br />
                                                            @endif{{ $userDetail->Qualification2 }}
                                                        @endif
                                                </td>
                                            </tr>
                                        @endif
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                @if ((bool) $pmsFile)
                                    <br>
                                    <a href="{{ url('filedownload') }}?file={{ $pmsFile }}" target="_blank"
                                        class="btn btn-xs btn-inverse-danger"><i class="fa fa-download"></i> Download Self
                                        Rated File</a>
                                    <br>
                                @endif
                                @if ((bool) $filePath3)
                                    <br>
                                    <a href="{{ url('filedownload') }}?file={{ $filePath3 }}" target="_blank"
                                        class="btn btn-xs btn-inverse-danger"><i class="fa fa-download"></i> Download Level
                                        1 Rated File</a>
                                    <br>
                                @endif
                                @if ((bool) $filePath4)
                                    <br>
                                    <a href="{{ url('filedownload') }}?file={{ $filePath4 }}" target="_blank"
                                        class="btn btn-xs btn-inverse-danger"><i class="fa fa-download"></i> Download Level
                                        2 Rated File</a>
                                    <br>
                                @endif
                                ...
                                @if ((bool) $filePath2)
                                    <br>
                                    <a href="{{ url('filedownload') }}?file={{ $filePath2 }}" target="_blank"
                                        class="btn btn-xs btn-inverse-danger"><i class="fa fa-download"></i> Download
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="ExcelApplicant">Goals/Targets {{ (bool) $pmsFile ? 'Re-upload' : 'Upload' }}
                                            file (optional) [5MB Max]</label>
                                        <input type="file"
                                            accept=".xls,.xlsx,.doc,.docx,.png,.jpg,.gif,.jpeg,.pdf,.ods,.ots,.odt,.ott,.oth,.odm"
                                            autocomplete="off" id="ExcelApplicant2"
                                            name="@if ((bool) $filePath3 && $type == 2 && !$hasNoLevel2) {{ 'File4' }}@else{{ 'File3' }} @endif"
                                            autocomplete="off" class="form-control file-xs" />
                                    </div>
                                </div>
                            </div>
                        @else
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-condensed less-padding" id="calc-total">
                                <thead>
                                    <tr>
                                        ...
                                        @if ($type == 2 && !$hasNoLevel2)
                                            <th class="text-center" style="width:16%;">Rating by @if (strlen($empDetails[0]->Level1Employee) > 30)
                                                    <br />
                                                @endif{!! $empDetails[0]->Level1Employee !!}</th>
                                        @endif
                                ...
                                <tbody>
                                    <?php $count = 1;
                                    $totalweightage = $selfRatingTotal = $level1RatingTotal = $level2RatingTotal = 0; ?>
                                        ...
                                        <tr>
                                            ...
                                            <td class="description">
                                                @if ($type == 1 || $empDetails[0]->Level2CriteriaType == 2 || $assessmentArea->ApplicableToLevel2 == 1 || $hasNoLevel2)
                                                    <input type="hidden"
                                                        name="pmssubmissiondetail[{{ $randomKey }}][Id]"
                                                        value="{{ $assessmentArea->Id }}" />
                                                @endif
                                            ...
                                            @if ($type == 2 && !$hasNoLevel2)
                                                <td class="text-center">
                                                    @if ($count == 1 && $level1UserExist)
                                                        &nbsp;
                                                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{ $assessmentArea->Level1Rating }}&nbsp;
                                                        <button type="button" class="btn btn-primary btn-xs"
                                                            id="subordinate-performance-goal-l2">
                                            ...
                                            $level1RatingTotal += $assessmentArea->Level1Rating; ?>
                                            <td @if ($count == 1) class="text-center" @endif>
                                                @if ($type == 1 || $empDetails[0]->Level2CriteriaType == 2 || $assessmentArea->ApplicableToLevel2 == 1 || $hasNoLevel2)
                                                    {{-- @if (count($multipleDetailArray) > 0)
                                                        ...
                                                        @else --}}
                                                    @if ($assessmentArea->ApplicableToLevel2 == 0 && $level1UserExist)
                                                        <div class="row mr-0 ml-0">
                                                            <div class="col-9 pl-0 text-center">
                                                                <input type="text" readonly="readonly"
                                                                    style="width:100%;"
                                                                    onkeydown="return event.keyCode !== 69"
                                                                    value="{{ $goalAchievementScore }}" min="0"
                                                                    max="{{ $assessmentArea->Weightage }}" step="any"
                                                                    required="required"
                                                                    class="form-control text-center input-xs figure"
                                                                    name="pmssubmissiondetail[{{ $randomKey }}][{{ $type == 1 || $hasNoLevel2 ? 'Level1Rating' : 'Level2Rating' }}]" />
                                                            </div>
                                                            <div class="col-3 pl-0 text-left">
                                                                <button type="button" class="btn btn-primary btn-xs"
                                                                    id="subordinate-performance-goal">
                                                                    <i class="fa fa-eye"
                                                    ...
                                                    @if ($assessmentArea->ApplicableToLevel2 == 1)
                                                        @if ($hasNoLevel2 || $level2UserExist)
                                                            <input type="number" onkeydown="return event.keyCode !== 69"
                                                                value="@if (in_array($empDetails[0]->LastStatusId, [CONST_PMSSTATUS_DRAFT])) {{ ($type == 1 || $hasNoLevel2) && (bool) $assessmentArea->Level1Rating ? $assessmentArea->Level1Rating : ((bool) $assessmentArea->Level2Rating ? $assessmentArea->Level2Rating : '') }}<?php if(!($type==1 && (bool)$assessmentArea->Level1Rating) && (bool)$assessmentArea->Level2Rating): ?><?php $level2RatingTotal += $assessmentArea->Level2Rating; ?><?php endif; ?> @endif"
                                                                name="pmssubmissiondetail[{{ $randomKey }}][{{ $type == 1 || $hasNoLevel2 ? 'Level1Rating' : 'Level2Rating' }}]"
                                                                min="0" max="{{ $assessmentArea->Weightage }}"
                                                                step="any" required="required"
                                                                class="form-control input-xs figure" />
                                                        @endif
                                                    @endif
                                                    {{-- @endif --}}
                                                @endif
                                        ...
                                        <td>
                                            <input type="text" autocomplete="off"
                                                @if (
                                                    ($type == 1 || $hasNoLevel2) &&
                                                        $level1RatingTotal > 0 &&
                                                        in_array($empDetails[0]->LastStatusId, [CONST_PMSSTATUS_DRAFT])) value="{{ number_format($level1RatingTotal, 2) }}"@else value="{{ number_format($level2RatingTotal, 2) }}" @endif
                                                class="form-control input-xs" id="figure-total" disabled="disabled" />
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        @if (($hasMultipleLevel1 && $loggedInLevel == 1) || ($hasMultipleLevel2 && $loggedInLevel == 2))
                            <button type="submit" name="SaveType" value="2"
                                class="dont-disable remove-required btn btn-primary">Save as Draft</button>
                            <button type="submit" name="SaveType" id="submit-process" value="1"
                                class="dont-disable btn btn-success">Submit</button>
                        ...
                        @else
                            <button type="button" name="SaveType" value="2" id="save-appraiser"
                                class="btn btn-primary">Save as Draft</button>
                            <button type="submit" name="SaveType" value="1" id="submit-process"
