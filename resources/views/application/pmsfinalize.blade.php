            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="col-sm-9 card dashboard-product">
                            ...
                            <div class="col-md-12">
                                <table style="width:100%;">
                                    @foreach ($details as $userDetail)
                                        <tr>
                                            <th colspan="2" class="text-center">
                                                @if ((bool) $userDetail->ProfilePicPath)
                                                    <a href="{{ asset((bool) $userDetail->ProfilePicPath ? $userDetail->ProfilePicPath : 'images/avatar.png') }}"
                                                        data-rel="lightcase">
                                                @endif
                                                <img class="rounded-circle img-thumbnail"
                                                    src="{{ asset((bool) $userDetail->ProfilePicPath ? $userDetail->ProfilePicPath : 'images/avatar.png') }}"
                                                    style="height:117px;" />
                                                @if ((bool) $userDetail->ProfilePicPath)
                                                    </a>
                                                @endif
                                        ...
                                        <tr>
                                            <th style="width:50%;">PayScale <span
                                                    class="pull-right d-none d-sm-block">:</span> &nbsp;&nbsp;</th>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                @if ((bool) $application[0]->FilePath)
                                    <br>
                                    <a href="{{ url('filedownload') }}?file={{ $application[0]->FilePath }}"
                                        target="_blank" class="btn btn-xs btn-inverse-danger"><i class="fa fa-download"></i>
                                        Download Self Rated File</a>
                                    <br>
                                @endif
                                @if ((bool) $application[0]->File3Path)
                                    <br>
                                    <a href="{{ url('filedownload') }}?file={{ $application[0]->File3Path }}"
                                        target="_blank" class="btn btn-xs btn-inverse-danger"><i class="fa fa-download"></i>
                                        Download Level 1 Rated File</a>
                                    <br>
                                @endif
                                @if ((bool) $application[0]->File4Path)
                                    <br>
                                    <a href="{{ url('filedownload') }}?file={{ $application[0]->File4Path }}"
                                        target="_blank" class="btn btn-xs btn-inverse-danger"><i class="fa fa-download"></i>
                                        Download Level 2 Rated File</a>
                                    <br>
                                @endif
                                @foreach ($pmsMultiple as $multiple)
                                    <br>
                                    <a href="{{ url('filedownload') }}?file={{ $multiple->FilePath }}" target="_blank"
                                        class="btn btn-xs btn-inverse-danger"><i class="fa fa-download"></i> Download
                                        {{ $multiple->ForLevel == 1 ? 'Level 1' : 'Level 2' }} Rated File (Uploaded by
                                        {{ $multiple->Appraiser }})</a>
                                    <br />
                                @endforeach
                                @if ((bool) $application[0]->File2Path)
                                    <br>
                                    <a href="{{ url('filedownload') }}?file={{ $application[0]->File2Path }}"
                                        target="_blank" class="btn btn-xs btn-inverse-danger"><i class="fa fa-download"></i>
                                        Download Supporting Document</a>
                                    <br><br>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="row">
                                    ...
                                    <select name="PMSOutcomeId" id="PMSOutComeId" class="form-control select2">
                                        @foreach ($outcomes as $outcome)
                                            <option value="{{ $outcome->Id }}"
                                                data-reference="{{ $outcome->ReferenceNo ?? '0' }}"
                                                data-designationlocationchange="{{ $outcome->HasDesignationAndLocationChange }}"
                                                data-basicpaychange="{{ $outcome->HasBasicPayChange }}"
                                                data-paychange="{{ $outcome->HasPayChange }}"
                                                data-positionchange="{{ $outcome->HasPositionChange }}"
                                                @if ($outcome->Id == $application[0]->PMSOutcomeId) selected="selected" @endif>
                                                {{ $outcome->Name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="number" readonly="readonly" autocomplete="off" class="form-control"
                                        name="OldBasicPay" id="OldBasicPay" value="{{ $details[0]->BasicPay }}" />
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="BasicPayScale" class="control-label">Basic Pay Scale</label>
                                    <input type="text" readonly="readonly" autocomplete="off" class="form-control"
                                        name="BasicPayScale" id="BasicPayScale" value="{{ $details[0]->PayScale }}" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-7 @if ($application[0]->HasPositionChange != 1) {{ 'hide' }} @endif"
                                id="PositionWrapper">
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <select data-payscaletype="{{ (int) $settingForPayScale }}"
                                                ...
                                                @if ((int) $settingForPayScale === 1)
                                                    @foreach ($gradesteps as $gradestep)
                                                        <option value="{{ $gradestep->Id }}"
                                                            data-basepay="{{ $gradestep->StartingSalary ?? '' }}"
                                                            data-lastpay="{{ $gradestep->EndingSalary ?? '' }}"
                                                            data-increment="{{ $gradestep->Increment ?? '' }}"
                                                            @if ($application[0]->NewGradeStepId == $gradestep->Id) selected="selected" @endif>
                                                            {{ $gradestep->GradeStep }}</option>
                                                    @endforeach
                                                @else
                                                    @foreach ($gradesteps as $gradestep)
                                                        <option value="{{ $gradestep->Id }}"
                                                            data-basepay="{{ $gradestep->StartingSalary ?? '' }}"
                                                            data-middlepay="{{ $gradestep->MiddleSalary ?? '' }}"
                                                            data-lastpay="{{ $gradestep->EndingSalary ?? '' }}"
                                                            data-firstincrement="{{ $gradestep->FirstIncrement ?? '' }}"
                                                            data-secondincrement="{{ $gradestep->SecondIncrement ?? '' }}"
                                                            @if ($application[0]->NewGradeStepId == $gradestep->Id) selected="selected" @endif>
                                                            {{ $gradestep->GradeStep }}</option>
                                                    @endforeach
                                                @endif
                                            ...
                                            <select name="NewGradeId" id="NewGradeId" class="form-control select2">
                                                ...
                                                @foreach ($grades as $grade)
                                                    <option value="{{ $grade->Id }}"
                                                        @if (
                                                            $grade->Id ==
                                                                ((bool) $application[0]->NewGradeId
                                                                    ? $application[0]->NewGradeId
                                                                    : (isset($employee['GradeId'])
                                                                        ? $employee['GradeId']
                                                                        : old('GradeId')))) selected="selected" @endif>
                                                        {{ $grade->Name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                            ...
                                            <select name="NewSupervisorId" id="Position" class="form-control select2">
                                                ...
                                                @foreach ($supervisors as $supervisor)
                                                    <option value="{{ $supervisor->Id }}"
                                                        @if (
                                                            $supervisor->Id ==
                                                                ((bool) $application[0]->NewSupervisorId
                                                                    ? $application[0]->NewSupervisorId
                                                                    : (isset($employee['SupervisorId'])
                                                                        ? $employee['SupervisorId']
                                                                        : old('SupervisorId')))) selected="selected" @endif>
                            ...
                            <div class="col-md-4 @if ($application[0]->HasBasicPayChange != 1) {{ 'hide' }} @endif"
                                id="PaySlabWrapper">
                                <div class="form-group">
                            ...
                            <div class="col-md-3 @if ($application[0]->HasPayChange != 1) {{ 'hide' }} @endif"
                                id="PayScaleWrapper">
                                <div class="form-group">
                                    <label for="PayScale" class="control-label">New Pay Scale</label>
                                    <input type="text" autocomplete="off" value="{{ $application[0]->NewPayScale }}"
                                        class="form-control" name="NewPayScale" id="PayScale" />
                                </div>
                            </div>
                            <div class="col-md-3 @if ($application[0]->HasBasicPayChange != 1) {{ 'hide' }} @endif"
                                id="BasicPayWrapper">
                                <div class="form-group">
                                    <label for="BasicPay" class="control-label">New Basic Pay</label>
                                    <input type="text" autocomplete="off" class="form-control" name="NewBasicPay"
                                        id="BasicPay" />
                                </div>
                            </div>
                        </div>

                        <div class="row @if ($application[0]->HasDesignationAndLocationChange != 1) {{ 'hide' }} @endif"
                            id="DesignationLocationWrapper">
                            <div class="col-md-7">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="Designation" class="control-label">New Designation</label>
                                            <select name="NewDesignationId" id="Designation"
                                                class="form-control select2">
                                                <option value="">N/A</option>
                                                @foreach ($designations as $designation)
                                                    <option value="{{ $designation->Id }}"
                                                        @if ($application[0]->NewDesignationId == $designation->Id) selected="selected" @endif>
                        ...
                        <input type="hidden" id="old-basic-pay"
                            value="{{ str_replace(',', '', $userDetail->BasicPay) }}" />
                        ...
                        <button type="submit" name="Submit" value="2"
                            class="btn btn-primary dont-disable">{{ (bool) $application[0]->PMSOutcomeId ? 'Update' : 'Submit' }}</button>
                        <a href="{{ url('appraisepms') }}" style="" class="btn btn-danger"><i
                                        ...
                                        <tbody>
                                            ...
                                            @forelse($history as $data)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{ $data->PMSScore }}</td>
                                                    <td>{{ $data->PMSResult }} @if ((bool) $data->PMSRemarks)
                                                            <br />{{ $data->PMSRemarks }}
                                                        @endif
                                                    </td>
                                                </tr>
                                            @empty
                                                <tr>
                                                    <td colspan="4">
                                                        <center>No PMS done till date.</center>
                                                    </td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                    ...
                                    <table class="table table-condensed table-bordered less-padding">
                                        ...
                                        <tbody>
                                            ...
                                            @forelse($disciplinaryDetails as $disciplinaryDetail)
                                                <tr>
                                                    <td>{{ $count++ }}</td>
                                                    <td>{{ convertDateToClientFormat($disciplinaryDetail->RecordDate) }}
                                                    </td>
